...
function setup() {
    ...
    Ground1= new ground(width/2,480,width,15)
    Ball= new Paper(100,420,10)
	
	//Create the Bodies Here.
	Launcher1= new Launcher(Ball.body,{x:100,y:400})
...
function keyPressed(){
	...
	//	 }
     ...
     async function getTime(){
        var info=await fetch("http://worldtimeapi.org/api/timezone/America/New_York"); 
        var info_in_json=await info.json();
    
        console.log(info_in_json);
    
        var time=info_in_json.datetime; console.log(time);
    
        var hr=time.slice(11,13); console.log(hr);
        if(hr<=6&& hr>= 18){
            bg="DarkBrown.jpg"
        }
